...
public class BillAnalysisService {
    ...
    @Autowired
    private MetadataForensicsService metadataForensicsService;

    @Autowired
    private BillDataExtractorService billDataExtractorService;

    @Autowired
    private GoogleMapsService googleMapsService;

    @Autowired
    private AiBillDetectorService aiBillDetectorService;
    ...
    public AnalysisResult analyze(MultipartFile file) {
        ...
        try {
            ...
            if (!isPdf && metadataForensicsService.hasEditingSoftwareMetadata(file)) {
                return buildResult("TAMPERED", 99, "Digital Manipulation Detected: File contains editing software footprints.", "HIGH", fileName);
            }
            String extractedText = ocrService.extractTextFromFile(file);

System.out.println("=== OCR OUTPUT ===\n" + extractedText + "\n=== END OCR ===");

// AI GENERATED BILL CHECK
if (aiBillDetectorService.isAiGenerated(extractedText)) {

    return buildResult(
            "AI_GENERATED",
            95,
            "Document appears to be AI generated.",
            "HIGH",
            fileName);
}
...
String lowerText = extractedText.toLowerCase();
    ...
    if (!(lowerText.contains("invoice")
        ...
        || lowerText.contains("gst")
        || lowerText.contains("tax invoice")
        || lowerText.contains("total"))) {

    return buildResult(
            "INVALID_DOCUMENT",
            100,
            "Uploaded file is not a bill or receipt.",
            "HIGH",
            fileName);
}
...
return runComplianceAudit(data, extractedText);
    ...
    private AnalysisResult runComplianceAudit(BillData data, String rawText) {
        ...
        if (text.contains("your business name") || text.contains("your street") ||
            ...
            return buildResult("TAMPERED", 99, "Document Forgery: Unedited internet receipt template detected.", "HIGH", data.vendor);
        }
        ...
        if (gstMatcher.find()) {
            ...
            if (stateCode < 1 || stateCode > 37) {
                return buildResult("TAMPERED", 95, "Invalid GST Number: " + gstNumber + " state code is not valid.", "HIGH", data.vendor);
            }
        ...
        try {
            ...
            if (invoiceDate != null && invoiceDate.isAfter(LocalDate.now())) {
                return buildResult("TAMPERED", 95, "Chronological Forgery: Invoice has a future date (" + data.date + ").", "HIGH", data.vendor);
            }
        } catch (Exception e) { }
        try {
            ...
            if (invDate2 != null) {
                ...
                if (invDate2.isBefore(LocalDate.now().minusDays(90)))
                    return buildResult("SUSPICIOUS", 72, "Expired Bill: Invoice date (" + data.date + ") is more than 90 days old.", "MEDIUM", data.vendor);
            }
        } catch (Exception e) { }
        ...
        try {
            ...
            boolean isCab = text.contains("uber") || text.contains("ola") || text.contains("taxi") || text.contains("cab");
            if (isCab && amt > 3000.00)
                return buildResult("SUSPICIOUS", 88, "Cab fare exceeds corporate single-ride allowance.", "MEDIUM", data.vendor);
        } catch (NumberFormatException e) { }
        if (!text.contains("uber") && !text.contains("ola") && "Unknown Flight".equals(data.flightNumber)) {
            if (!verifyVendor(data.vendor))
                return buildResult("TAMPERED", 96, "Vendor '" + data.vendor + "' could not be verified on Google Maps.", "HIGH", data.vendor);
        }
        try {
            double amt = Double.parseDouble(data.amount.replace(",", ""));
            double marketRate = getMarketRate(data.vendor);
            if (amt > marketRate * 2.0)
                return buildResult("SUSPICIOUS", 85, "Price (" + data.amount + ") far exceeds regional average (" + marketRate + ").", "MEDIUM", data.vendor);
            if (amt > 50000.00)
                return buildResult("SUSPICIOUS", 80, "Amount exceeds automated company limits.", "MEDIUM", data.vendor);
        } catch (NumberFormatException e) { }
        if ("Unknown Date".equals(data.date) || "0.00".equals(data.amount))
            return buildResult("SUSPICIOUS", 70, "Crucial fields could not be read from the document.", "MEDIUM", data.vendor);
        String gstInfo = "NOT FOUND".equals(gstNumber) ? " No GST number found on bill." : " GST " + gstNumber + " verified valid.";
    ...
    private AnalysisResult buildResult(String status, int confidence, String reason, String riskLevel, String vendorHint) {
